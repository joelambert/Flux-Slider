...
this.options.height:null;this.width=this.options.width?this.options.width:null;if(!flux.browser.supports3d){var e=[];$(this.options.transitions).each(function(f,g){(new flux.transitions[g](this)).options.requires3d||e.push(g)});this.options.transitions=e}this.images=[];this.currentImageIndex=this.imageLoadedCount=0;this.nextImageIndex=1;this.playing=false;this.element.find("img").each(function(f,g){a.images.push(g.cloneNode(false));var h=new Image;h.onload=function(){a.imageLoadedCount++;a.width=
a.width?a.width:this.width;a.height=a.height?a.height:this.height;if(a.imageLoadedCount>=a.images.length){a.finishedLoading();a.setupImages()}};h.src=g.src;$(g).remove()});this.container=$('<div class="fluxslider"></div>').appendTo(this.element);this.imageContainer=$('<div class="images loading"></div>').css({position:"relative",overflow:"hidden","min-height":"100px"}).appendTo(this.container);this.image1=$('<div class="image1" style="height: 100%; width: 100%"></div>').appendTo(this.imageContainer);
this.image2=$('<div class="image2" style="height: 100%; width: 100%"></div>').appendTo(this.imageContainer);$(this.image1).add(this.image2).css({position:"absolute",top:"0px",left:"0px"});this.options.onTransitionEnd&&this.element.bind("fluxTransitionEnd",function(f){f.preventDefault();a.options.onTransitionEnd(f.data)});this.options.autoplay&&this.start()};
