...
flux.slider=function(b,d){if(flux.browser.supportsTransitions){var a=this;this.element=$(b);this.transitions=[];for(var c in flux.transitions)this.transitions.push(c);this.options=$.extend({autoplay:true,transitions:this.transitions,delay:4E3,pagination:true,controls:true},d);this.images=[];this.currentImageIndex=this.imageLoadedCount=0;this.nextImageIndex=1;this.playing=false;this.element.find("img").each(function(e,f){a.images.push(f.cloneNode(false));var g=new Image;g.onload=function(){a.imageLoadedCount++;
...
flux.browser={translate:function(b,d,a){b=b!=undefined?b:0;d=d!=undefined?d:0;a=a!=undefined?a:0;return"translate"+(flux.browser.supports3d?"3d(":"(")+b+"px,"+d+(flux.browser.supports3d?"px,"+a+"px)":"px)")},rotateX:function(b){return flux.browser.rotate("x",b)},rotateY:function(b){return flux.browser.rotate("y",b)},rotateZ:function(b){return flux.browser.rotate("z",b)},rotate:function(b,d){if(!b in{x:"",y:"",z:""})b="z";d=d!=undefined?d:0;return flux.browser.supports3d?"rotate3d("+(b=="x"?"1":"0")+
", "+(b=="y"?"1":"0")+", "+(b=="z"?"1":"0")+", "+d+"deg)":b=="z"?"rotate("+d+"deg)":""}};
$(function(){var b=document.createElement("div");flux.browser.supportsTransitions=false;for(var d=["Webkit","Moz","O","Ms"],a=0;a<d.length;a++)if(d[a]+"Transition"in b.style)flux.browser.supportsTransitions=flux.browser.supportsTransitions||true;flux.browser.supports3d="WebKitCSSMatrix"in window&&"m11"in new WebKitCSSMatrix;if(flux.browser.supports3d&&"webkitPerspective"in b.style){b=$('<div id="csstransform3d"></div>');d=$('<style media="(transform-3d), (-webkit-transform-3d)">div#csstransform3d { position: absolute; left: 9px }</style>');
$("body").append(b);$("head").append(d);flux.browser.supports3d=b.get(0).offsetLeft==9;b.remove();d.remove()}});(function(){$.fn.css3=function(b){var d={},a=["webkit","moz","ms","o"],c;for(c in b)for(var e=0;e<a.length;e++)d["-"+a[e]+"-"+c]=b[c];this.css(d);return this};$.fn.transitionEnd=function(b){for(var d=["webkitTransitionEnd","transitionend","oTransitionEnd"],a=0;a<d.length;a++)this.bind(d[a],function(c){for(var e=0;e<d.length;e++)$(this).unbind(d[e]);b&&b.call(this,c)});return this}})();
...
flux.transitions={};
flux.transitions.bars=function(b,d){return new flux.transition(b,$.extend({barWidth:60,setup:function(){for(var a=Math.floor(this.slider.image1.width()/this.options.barWidth)+1,c=0;c<a;c++)this.slider.image1.append($("<div></div>").attr("class","bar bar-"+c).css({width:this.options.barWidth+"px",height:"100%",position:"absolute",top:"0",left:c*this.options.barWidth+"px","background-image":this.slider.image1.css("background-image"),"background-position":"-"+c*this.options.barWidth+"px 0px"}).css3({"transition-duration":"400ms",
"transition-timing-function":"ease-in","transition-property":"all","transition-delay":c*40+"ms"}))},execute:function(){var a=this,c=this.slider.image1.height(),e=this.slider.image1.find("div.bar");$(e[e.length-1]).transitionEnd(function(){a.finished()});e.css({opacity:"0.5"}).css3({transform:flux.browser.translate(0,c)})}},d))};
flux.transitions.bars3d=function(b,d){return new flux.transition(b,$.extend({requires3d:true,barWidth:100,perspective:600,setup:function(){var a=Math.floor(this.slider.image1.width()/this.options.barWidth)+1;this.options.barWidth=Math.floor(this.slider.image1.width()/a);for(var c=this.slider.image1.width()-a*this.options.barWidth,e=Math.ceil(c/a),f=this.slider.image1.height(),g=0,h=0;h<a;h++){var i=this.options.barWidth;if(c>0){var j=c>=e?e:c;i+=j;c-=j}j=$('<div class="bar bar-'+h+'"></div>').css({width:i+
"px",height:"100%",position:"absolute",top:"0px",left:"0px","z-index":200,"background-image":this.slider.image1.css("background-image"),"background-position":"-"+g+"px 0px","background-repeat":"no-repeat"}).css3({"backface-visibility":"hidden"});var k=$(j.get(0).cloneNode(false)).css({"background-image":this.slider.image2.css("background-image")}).css3({transform:flux.browser.rotateX(90)+" "+flux.browser.translate(0,-f/2,f/2)}),n=$('<div class="side bar bar-'+h+'"></div>').css({width:f+"px",height:f+
"px",position:"absolute",top:"0px",left:"0px",background:"#222","z-index":190}).css3({transform:flux.browser.rotateY(90)+" "+flux.browser.translate(f/2,0,-f/2)+" "+flux.browser.rotateY(180),"backface-visibility":"hidden"}),o=$(n.get(0).cloneNode(false)).css3({transform:flux.browser.rotateY(90)+" "+flux.browser.translate(f/2,0,i-f/2)});this.slider.image1.append($('<div class="barcontainer"></div>').css({width:i+"px",height:"100%",position:"absolute",top:"0px",left:g+"px","z-index":h>a/2?1E3-h:1E3}).css3({"transition-duration":"800ms",
"transition-timing-function":"linear","transition-property":"all","transition-delay":h*150+"ms","transform-style":"preserve-3d"}).append(j).append(k).append(n).append(o));g+=i}this.slider.imageContainer.css3({perspective:this.options.perspective,"perspective-origin":"50% 50%"})},execute:function(){var a=this,c=this.slider.image1.height(),e=this.slider.image1.find("div.barcontainer");this.slider.image2.hide();e.last().transitionEnd(function(){a.slider.image2.show();a.finished()});e.css3({transform:flux.browser.rotateX(-90)+
" "+flux.browser.translate(0,c/2,c/2)})}},d))};flux.transitions.blinds=function(b,d){return new flux.transitions.bars(b,$.extend({barWidth:70,execute:function(){var a=this;this.slider.image1.height();var c=this.slider.image1.find("div.bar");$(c[c.length-1]).transitionEnd(function(){a.finished()});c.css({opacity:"0.5"}).css3({transform:"scalex(0.0001)"})}},d))};
flux.transitions.blinds3d=function(b,d){return new flux.transition(b,$.extend({requires3d:true,barWidth:150,perspective:600,setup:function(){var a=Math.floor(this.slider.image1.width()/this.options.barWidth)+1;this.slider.image1.height();this.options.barWidth=Math.floor(this.slider.image1.width()/a);for(var c=this.slider.image1.width()-a*this.options.barWidth,e=Math.ceil(c/a),f=this.slider.image1.height(),g=0,h=0;h<a;h++){var i,j,k;if(c>0){i=c>=e?e:remainderY;c-=i}i=this.options.barWidth;j=$('<div class="bar bar-'+
h+'"></div>').css({width:i+"px",height:"100%",position:"absolute",top:"0px",left:"0px","z-index":200,"background-image":this.slider.image1.css("background-image"),"background-position":"-"+g+"px 0px","background-repeat":"no-repeat"}).css3({"backface-visibility":"hidden"});k=$(j.get(0).cloneNode(false)).css({"background-image":this.slider.image2.css("background-image"),"z-index":190}).css3({transform:flux.browser.rotateY(180)});this.slider.image1.append($('<div class="barcontainer"></div>').css({width:i+
"px",height:f+"px",position:"absolute",top:"0px",left:g+"px","z-index":h>a/2?1E3-h:1E3}).css3({"transition-duration":"800ms","transition-timing-function":"ease-out","transition-property":"all","transition-delay":h*150+"ms","transform-style":"preserve-3d"}).append(j).append(k));g+=i}this.slider.imageContainer.css3({perspective:this.options.perspective,"perspective-origin":"50% 50%"})},execute:function(){var a=this,c=this.slider.image1.find("div.barcontainer");this.slider.image2.hide();c.last().transitionEnd(function(){a.slider.image2.show();
a.finished()});c.css3({transform:flux.browser.rotateY(180)})}},d))};flux.transitions.zip=function(b,d){return new flux.transitions.bars(b,$.extend({execute:function(){var a=this,c=this.slider.image1.height(),e=this.slider.image1.find("div.bar");$(e[e.length-1]).transitionEnd(function(){a.finished()});e.each(function(f,g){setTimeout(function(){$(g).css({opacity:"0.3"}).css3({transform:flux.browser.translate(0,f%2?"-"+2*c:c)})},5)})}},d))};
flux.transitions.blocks=function(b,d){return new flux.transition(b,$.extend({blockSize:80,blockDelays:{},maxDelay:0,setup:function(){for(var a=Math.floor(this.slider.image1.width()/this.options.blockSize)+1,c=Math.floor(this.slider.image1.height()/this.options.blockSize)+1,e=0;e<a;e++)for(var f=0;f<c;f++){var g=Math.floor(Math.random()*10*100),h=$("<div></div>").attr("class","block block-"+e+"-"+f).data("id",e+":"+f).css({width:this.options.blockSize+"px",height:this.options.blockSize+"px",position:"absolute",
top:f*this.options.blockSize+"px",left:e*this.options.blockSize+"px","background-image":this.slider.image1.css("background-image"),"background-position":"-"+e*this.options.blockSize+"px -"+f*this.options.blockSize+"px"}).css3({"transition-duration":"350ms","transition-timing-function":"ease-in","transition-property":"all","transition-delay":g+"ms"});this.slider.image1.append(h);if(g>this.options.maxDelay){this.options.maxDelayBlock=h;this.options.maxDelay=g}}},execute:function(){var a=this,c=this.slider.image1.find("div.block");
this.options.maxDelayBlock.transitionEnd(function(){a.finished()});c.each(function(e,f){setTimeout(function(){$(f).css({opacity:"0"}).css3({transform:"scale(0.8)"})},5)})}},d))};
flux.transitions.concentric=function(b,d){return new flux.transition(b,$.extend({blockSize:60,delay:150,alternate:false,setup:function(){for(var a=this.slider.image1.width(),c=this.slider.image1.height(),e=Math.ceil((Math.sqrt(a*a+c*c)-this.options.blockSize)/2/this.options.blockSize)+1,f=0;f<e;f++){var g=2*f*this.options.blockSize+this.options.blockSize;this.slider.image1.append($("<div></div>").attr("class","block block-"+f).css({width:g+"px",height:g+"px",position:"absolute",top:(c-g)/2+"px",left:(a-
g)/2+"px","z-index":100+(e-f),"background-image":this.slider.image1.css("background-image"),"background-position":"center center"}).css3({"border-radius":"1000px","transition-duration":"800ms","transition-timing-function":"linear","transition-property":"all","transition-delay":(e-f)*this.options.delay+"ms"}))}},execute:function(){var a=this,c=this.slider.image1.find("div.block");$(c[0]).transitionEnd(function(){a.finished()});c.each(function(e,f){setTimeout(function(){$(f).css({opacity:"0"}).css3({transform:flux.browser.rotateZ((!a.options.alternate||
e%2?"":"-")+"90")})},5)})}},d))};flux.transitions.warp=function(b,d){return new flux.transitions.concentric(b,$.extend({delay:30,alternate:true},d))};
...
for(var i=0,j=$("<div></div>"),k=0;k<a;k++){var n=0,o=this.options.tileWidth;if(e>0){var l=e>=f?f:e;o+=l;e-=l}for(l=0;l<c;l++){var p=this.options.tileWidth,m=g;if(m>0)p+=m>=h?h:m;m=$('<div class="tile tile-'+k+"-"+l+'"></div>').css({width:o+"px",height:p+"px",position:"absolute",top:"0px",left:"0px","z-index":200,"background-image":this.slider.image1.css("background-image"),"background-position":"-"+i+"px -"+n+"px","background-repeat":"no-repeat"}).css3({"backface-visibility":"hidden"});var q=$(m.get(0).cloneNode(false)).css({"background-image":this.slider.image2.css("background-image"),
"z-index":190}).css3({transform:flux.browser.rotateY(180)});m=$('<div class="tilecontainer"></div>').css({width:o+"px",height:p+"px",position:"absolute",top:n+"px",left:i+"px","z-index":(k>a/2?500-k:500)+(l>c/2?500-l:500)}).css3({"transition-duration":"800ms","transition-timing-function":"ease-out","transition-property":"all","transition-delay":k*200+l*150+"ms","transform-style":"preserve-3d"}).append(m).append(q);$(j).append(m);n+=p}i+=o}this.slider.image1.append(j);this.slider.imageContainer.css3({perspective:this.options.perspective,
"perspective-origin":"50% 50%"})},execute:function(){var a=this,c=this.slider.image1.find("div.tilecontainer");this.slider.image2.hide();c.last().transitionEnd(function(){a.slider.image2.show();a.finished()});c.css3({transform:flux.browser.rotateY(180)})}},d))};
