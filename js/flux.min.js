...
flux.slider=function(b,d){flux.browser.init();if(!flux.browser.supportsTransitions){window.console&&window.console.error&&console.error("Flux Slider requires a browser that supports CSS3 transitions");return false}var a=this;this.element=$(b);this.transitions=[];for(var c in flux.transitions)this.transitions.push(c);this.options=$.extend({autoplay:true,transitions:this.transitions,delay:4E3,pagination:true,controls:true,width:null,height:null},d);this.height=this.options.height?this.options.height:
null;this.width=this.options.width?this.options.width:null;if(!flux.browser.supports3d){var e=[];$(this.options.transitions).each(function(f,g){(new flux.transitions[g](this)).options.requires3d||e.push(g)});this.options.transitions=e}this.images=[];this.currentImageIndex=this.imageLoadedCount=0;this.nextImageIndex=1;this.playing=false;this.element.find("img").each(function(f,g){a.images.push(g.cloneNode(false));var h=new Image;h.onload=function(){a.imageLoadedCount++;a.width=a.width?a.width:this.width;
a.height=a.height?a.height:this.height;if(a.imageLoadedCount>=a.images.length){a.finishedLoading();a.setupImages()}};h.src=g.src;$(g).remove()});this.container=$('<div class="fluxslider"></div>');this.element.append(this.container);this.imageContainer=$('<div class="images loading"></div>').css({position:"relative",overflow:"hidden","min-height":"100px"});this.container.append(this.imageContainer);this.image1=$('<div class="image1" style="height: 100%; width: 100%"></div>');this.imageContainer.append(this.image1);
this.image2=$('<div class="image2" style="height: 100%; width: 100%"></div>');this.imageContainer.append(this.image2);$(this.image1).css({position:"absolute",top:"0px",left:"0px"});$(this.image2).css({position:"absolute",top:"0px",left:"0px"});this.options.autoplay&&this.start()};
...
"right center"});$("<div></div>").appendTo(a).css({"background-image":this.slider.image1.css("background-image"),"background-position":(this.options.direction=="left"?"-"+this.slider.image1.width()/2:0)+"px 0",width:"100%",height:"100%",position:"absolute",top:"0",left:"0"});$("<div></div>").appendTo(a).css({"background-image":this.slider.image2.css("background-image"),"background-position":(this.options.direction=="left"?0:"-"+this.slider.image1.width()/2)+"px 0",width:"100%",height:"100%",position:"absolute",
top:"0",left:"0"}).css3({transform:flux.browser.rotateY(180),"backface-visibility":"hidden"});var c=$("<div></div>").css({position:"absolute",top:"0",left:this.options.direction=="left"?"0":"50%",width:"50%",height:"100%","background-image":this.slider.image1.css("background-image"),"background-position":(this.options.direction=="left"?0:"-"+this.slider.image1.width()/2)+"px 0","z-index":100}),e=$('<div class="overlay"></div>').css({position:"absolute",top:"0",left:this.options.direction=="left"?
"50%":"0",width:"50%",height:"100%",background:"#000",opacity:1}).css3({"transition-duration":"800ms","transition-timing-function":"linear","transition-property":"opacity"});this.slider.image1.append($("<div></div>").css3({width:"100%",height:"100%"}).css3({perspective:this.options.perspective,"perspective-origin":"50% 50%"}).append(a).append(c).append(e))},execute:function(){var a=this;this.slider.image1.find("div.tab").first().transitionEnd(function(){a.finished()}).css3({transform:flux.browser.rotateY(this.options.direction==
"left"?-180:180)}).end().end().find("div.overlay").css({opacity:0})}},d))};
