...
this.getImage(this.currentImageIndex).src+'")',"z-index":101}).children().remove();this.image2.css({"background-image":'url("'+this.getImage(this.nextImageIndex).src+'")',"z-index":100});if(this.options.pagination){this.pagination.find("li.current").removeClass("current");$(this.pagination.find("li")[this.currentImageIndex]).addClass("current")}},transition:function(a,c){if(a==undefined||!flux.transitions[a])a=this.options.transitions[Math.floor(Math.random()*this.options.transitions.length)];(new flux.transitions[a](this,
$.extend(this.options[a]?this.options[a]:{},c))).run();this.currentImageIndex=this.nextImageIndex;this.setNextIndex(this.currentImageIndex+1)},getImage:function(a){a%=this.images.length;return this.images[a]},setNextIndex:function(a){if(a==undefined)a=this.currentImageIndex+1;this.nextImageIndex=a;if(this.nextImageIndex>this.images.length-1)this.nextImageIndex=0;if(this.nextImageIndex<0)this.nextImageIndex=this.images.length-1},increment:function(){this.currentImageIndex++;if(this.currentImageIndex>
this.images.length-1)this.currentImageIndex=0}};
...
"transition-timing-function":"linear","transition-property":"all","transition-delay":h*150+"ms","transform-style":"preserve-3d"}).append(j).append(l).append(k).append(m));g+=i}this.imageContainerOverflow=this.slider.imageContainer.css("overflow");this.slider.imageContainer.css({overflow:"visible"}).css3({perspective:this.options.perspective,"perspective-origin":"50% 50%"})},execute:function(){var b=this,d=this.slider.image1.height(),e=this.slider.image1.find("div.barcontainer");this.slider.image2.hide();
$(e[e.length-1]).transitionEnd(function(){b.slider.image2.show();b.slider.imageContainer.css({overflow:b.imageContainerOverflow});b.finished()});this.slider.image1.find("div.barcontainer").css3({transform:flux.browser.rotateX(-90)+" "+flux.browser.translate(0,d/2,d/2)})}},c))};
flux.transitions.blinds=function(a,c){return new flux.transitions.bars(a,$.extend({barWidth:70,execute:function(){var b=this;this.slider.image1.height();var d=this.slider.image1.find("div.bar");$(d[d.length-1]).transitionEnd(function(){b.finished()});d.css({opacity:"0.5"}).css3({transform:"scalex(0.0001)"})}},c))};
...
flux.transitions.cube=function(a,c){return new flux.transition(a,$.extend({requires3d:true,barWidth:100,direction:"left",perspective:1E3,setup:function(){var b=this.slider.image1.width(),d=this.slider.image1.height();this.imageContainerOverflow=this.slider.imageContainer.css("overflow");this.slider.imageContainer.css({overflow:"visible"}).css3({perspective:this.options.perspective,"perspective-origin":"50% 50%"});this.cubeContainer=$('<div class="cube"></div>').css({width:b+"px",height:d+"px",position:"relative"}).css3({"transition-duration":"800ms",
"transition-timing-function":"linear","transition-property":"all","transform-style":"preserve-3d"});b={height:"100%",width:"100%",position:"absolute",top:"0px",left:"0px"};this.cubeContainer.append($('<div class="face current"></div>').css($.extend(b,{background:this.slider.image1.css("background-image")})));this.cubeContainer.append($('<div class="face next"></div>').css($.extend(b,{background:this.slider.image2.css("background-image")})).css3({transform:this.options.transitionStrings.call(this,
this.options.direction,"nextFace")}));this.slider.image1.append(this.cubeContainer)},execute:function(){var b=this;this.slider.image1.width();this.slider.image1.height();this.slider.image2.hide();this.cubeContainer.css3({transform:this.options.transitionStrings.call(this,this.options.direction,"container")}).transitionEnd(function(){b.slider.image2.show();b.slider.imageContainer.css({overflow:b.imageContainerOverflow});b.finished()})},transitionStrings:function(b,d){var e=this.slider.image1.width(),
f=this.slider.image1.height();e={up:{nextFace:flux.browser.rotateX(-90)+" "+flux.browser.translate(0,f/2,f/2),container:flux.browser.rotateX(90)+" "+flux.browser.translate(0,-f/2,f/2)},down:{nextFace:flux.browser.rotateX(90)+" "+flux.browser.translate(0,-f/2,f/2),container:flux.browser.rotateX(-90)+" "+flux.browser.translate(0,f/2,f/2)},left:{nextFace:flux.browser.rotateY(90)+" "+flux.browser.translate(e/2,0,e/2),container:flux.browser.rotateY(-90)+" "+flux.browser.translate(-e/2,0,e/2)},right:{nextFace:flux.browser.rotateY(-90)+
" "+flux.browser.translate(-e/2,0,e/2),container:flux.browser.rotateY(90)+" "+flux.browser.translate(e/2,0,e/2)}};return e[b]&&e[b][d]?e[b][d]:false}},c))};
