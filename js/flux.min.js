...
*/
var flux={version:"1.3.3 (pre release)"};
flux.slider=function(b,d){flux.browser.init();if(!flux.browser.supportsTransitions){window.console&&window.console.error&&console.error("Flux Slider requires a browser that supports CSS3 transitions");return false}var a=this;this.element=$(b);this.transitions=[];for(var c in flux.transitions)this.transitions.push(c);this.options=$.extend({autoplay:true,transitions:this.transitions,delay:4E3,pagination:true,controls:true,width:null,height:null,onTransitionEnd:null},d);this.height=this.options.height?
this.options.height:null;this.width=this.options.width?this.options.width:null;if(!flux.browser.supports3d){var e=[];$(this.options.transitions).each(function(f,g){(new flux.transitions[g](this)).options.requires3d||e.push(g)});this.options.transitions=e}this.images=[];this.currentImageIndex=this.imageLoadedCount=0;this.nextImageIndex=1;this.playing=false;this.element.find("img, a img").each(function(f,g){var h=g.cloneNode(false),i=$(g).parent();i.is("a")&&$(h).data("href",i.attr("href"));a.images.push(h);
h=new Image;h.onload=function(){a.imageLoadedCount++;a.width=a.width?a.width:this.width;a.height=a.height?a.height:this.height;if(a.imageLoadedCount>=a.images.length){a.finishedLoading();a.setupImages()}};h.src=g.src;$(g).remove()});this.container=$('<div class="fluxslider"></div>').appendTo(this.element);this.container.bind("click",function(f){if($(f.target).hasClass("hasLink"))window.location=$(f.target).data("href")});this.imageContainer=$('<div class="images loading"></div>').css({position:"relative",
overflow:"hidden","min-height":"100px"}).appendTo(this.container);this.image1=$('<div class="image1" style="height: 100%; width: 100%"></div>').appendTo(this.imageContainer);this.image2=$('<div class="image2" style="height: 100%; width: 100%"></div>').appendTo(this.imageContainer);$(this.image1).add(this.image2).css({position:"absolute",top:"0px",left:"0px"});this.options.onTransitionEnd&&this.element.bind("fluxTransitionEnd",function(f){f.preventDefault();a.options.onTransitionEnd(f.data)});this.options.autoplay&&
this.start()};
...
"margin-left":"0.5em",cursor:"pointer"}).appendTo(b.pagination);d==0&&a.css("margin-left",0).addClass("current")});this.container.append(this.pagination)}$(this.imageContainer).css({width:this.width+"px",height:this.height+"px"});$(this.image1).css({width:this.width+"px",height:this.height+"px"});$(this.image2).css({width:this.width+"px",height:this.height+"px"});this.container.css({width:this.width+"px",height:this.height+(this.options.pagination?this.pagination.height():0)+"px"})},setupImages:function(){var b=
this.getImage(this.currentImageIndex),d={"background-image":'url("'+b.src+'")',"z-index":101,cursor:"auto"};if($(b).data("href")){d.cursor="pointer";this.image1.addClass("hasLink");this.image1.data("href",$(b).data("href"))}else{this.image1.removeClass("hasLink");this.image1.data("href",null)}this.image1.css(d).children().remove();this.image2.css({"background-image":'url("'+this.getImage(this.nextImageIndex).src+'")',"z-index":100}).show();if(this.options.pagination){this.pagination.find("li.current").removeClass("current");
$(this.pagination.find("li")[this.currentImageIndex]).addClass("current")}},transition:function(b,d){if(b==undefined||!flux.transitions[b])b=this.options.transitions[Math.floor(Math.random()*this.options.transitions.length)];(new flux.transitions[b](this,$.extend(this.options[b]?this.options[b]:{},d))).run();this.currentImageIndex=this.nextImageIndex;this.setNextIndex(this.currentImageIndex+1)},getImage:function(b){b%=this.images.length;return this.images[b]},setNextIndex:function(b){if(b==undefined)b=
this.currentImageIndex+1;this.nextImageIndex=b;if(this.nextImageIndex>this.images.length-1)this.nextImageIndex=0;if(this.nextImageIndex<0)this.nextImageIndex=this.images.length-1},increment:function(){this.currentImageIndex++;if(this.currentImageIndex>this.images.length-1)this.currentImageIndex=0}};
...
flux.transitions.turn=function(b,d){return new flux.transition(b,$.extend({requires3d:true,perspective:1300,direction:"left",setup:function(){var a=$('<div class="tab"></div>').css({width:"50%",height:"100%",position:"absolute",top:"0px",left:this.options.direction=="left"?"50%":"0%","z-index":101}).css3({"transform-style":"preserve-3d","transition-duration":"1000ms","transition-timing-function":"ease-out","transition-property":"all","transform-origin":this.options.direction=="left"?"left center":
"right center"});$("<div></div>").appendTo(a).css({"background-image":this.slider.image1.css("background-image"),"background-position":(this.options.direction=="left"?"-"+this.slider.image1.width()/2:0)+"px 0",width:"100%",height:"100%",position:"absolute",top:"0",left:"0"}).css3({"backface-visibility":"hidden"});$("<div></div>").appendTo(a).css({"background-image":this.slider.image2.css("background-image"),"background-position":(this.options.direction=="left"?0:"-"+this.slider.image1.width()/2)+
"px 0",width:"100%",height:"100%",position:"absolute",top:"0",left:"0"}).css3({transform:flux.browser.rotateY(180),"backface-visibility":"hidden"});var c=$("<div></div>").css({position:"absolute",top:"0",left:this.options.direction=="left"?"0":"50%",width:"50%",height:"100%","background-image":this.slider.image1.css("background-image"),"background-position":(this.options.direction=="left"?0:"-"+this.slider.image1.width()/2)+"px 0","z-index":100}),e=$('<div class="overlay"></div>').css({position:"absolute",
top:"0",left:this.options.direction=="left"?"50%":"0",width:"50%",height:"100%",background:"#000",opacity:1}).css3({"transition-duration":"800ms","transition-timing-function":"linear","transition-property":"opacity"});this.slider.image1.append($("<div></div>").css3({width:"100%",height:"100%"}).css3({perspective:this.options.perspective,"perspective-origin":"50% 50%"}).append(a).append(c).append(e))},execute:function(){var a=this;this.slider.image1.find("div.tab").first().transitionEnd(function(){a.finished()}).css3({transform:flux.browser.rotateY(this.options.direction==
"left"?-180:180)});this.slider.image1.find("div.overlay").css({opacity:0})}},d))};
flux.transitions.slide=function(b,d){return new flux.transition(b,$.extend({direction:"left",setup:function(){var a=this.slider.image1.width(),c=this.slider.image1.height(),e=$('<div class="current"></div>').css({height:c+"px",width:a+"px",position:"absolute",top:"0px",left:"0px",background:this.slider[this.options.direction=="left"?"image1":"image2"].css("background-image")}),f=$('<div class="next"></div>').css({height:c+"px",width:a+"px",position:"absolute",top:"0px",left:a+"px",background:this.slider[this.options.direction==
"left"?"image2":"image1"].css("background-image")});this.slideContainer=$('<div class="slide"></div>').css({width:2*a+"px",height:c+"px",position:"relative",left:this.options.direction=="left"?"0px":-a+"px"}).css3({"transition-duration":"600ms","transition-timing-function":"ease-in","transition-property":"all"});this.slideContainer.append(e).append(f);this.slider.image1.append(this.slideContainer)},execute:function(){var a=this,c=this.slider.image1.width();if(this.options.direction=="left")c=-c;this.slideContainer.css3({transform:flux.browser.translate(c)}).transitionEnd(function(){a.finished()})}},
d))};
