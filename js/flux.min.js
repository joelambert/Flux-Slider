...
flux.slider.prototype={constructor:flux.slider,start:function(){var a=this;this.interval=setInterval(function(){a.transition()},this.options.delay)},stop:function(){clearInterval(this.interval)},next:function(a){this.showImage(this.currentImageIndex+1,a)},prev:function(a){this.showImage(this.currentImageIndex-1,a)},showImage:function(a,c){this.setNextIndex(a);this.stop();this.setupImages();this.transition(c);this.options.autoplay&&this.start()},finishedLoading:function(){var a=this;this.container.css({width:this.width+
...
'")',"z-index":101}).children().remove();this.image2.css({"background-image":'url("'+this.getImage(this.nextImageIndex).src+'")',"z-index":100});if(this.options.pagination){this.pagination.find("li.current").removeClass("current");$(this.pagination.find("li")[this.currentImageIndex]).addClass("current")}},transition:function(a){if(a==undefined||!flux.transitions[a])a=this.options.transitions[Math.floor(Math.random()*this.options.transitions.length)];(new flux.transitions[a](this,this.options[a])).run();
this.currentImageIndex=this.nextImageIndex;this.setNextIndex(this.currentImageIndex+1)},getImage:function(a){a%=this.images.length;return this.images[a]},setNextIndex:function(a){if(a==undefined)a=this.currentImageIndex+1;this.nextImageIndex=a;if(this.nextImageIndex>this.images.length-1)this.nextImageIndex=0;if(this.nextImageIndex<0)this.nextImageIndex=this.images.length-1},increment:function(){this.currentImageIndex++;if(this.currentImageIndex>this.images.length-1)this.currentImageIndex=0}};
